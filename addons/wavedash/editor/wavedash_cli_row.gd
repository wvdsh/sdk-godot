...
## Once per session: unauthenticated GitHub calls share a 60/hour/IP budget. Set only on success, so
## being offline doesn't burn the attempt.
const UPDATE_CHECKED_KEY := "cli_update_checked"

## Checks the releases endpoint rather than running `wavedash update`, which has no dry-run flag.
func _check_for_update_async() -> void:
	...
	# request() returns ERR_UNCONFIGURED in the same step as add_child(). Deferred rather than awaited:
	# a @tool script suspended mid-coroutine doesn't survive this file being saved.
...
## Tolerates a "v" prefix: "v1".to_int() is 0, which would make v1.0.1 compare as older than 1.0.0.
